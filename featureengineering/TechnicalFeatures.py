...
class TechnicalFeatures:
    ...
    def create_candle_pattern_features(self, data: pd.DataFrame) -> pd.DataFrame:
        """
    Додає свічкові патерни та пов’язані з ними ознаки до датафрейму OHLCV.

    Метод виявляє та кодує десятки популярних одно-, дво- та три-свічкових патернів,
    а також кастомні патерни на основі логіки свічок (довгі тіні, великі тіла,
    пін-бари, high-wave, firefly, inside bar тощо). Патерни векторизовані
    для ефективної обробки великих таймсерій.

    Всі булеві патерни перетворюються в цілочисельні змінні (0/1), придатні
    для машинного навчання. Більшість патернів враховують відношення ціни закриття
    до відкриття, довжину тіла свічки та тіні, а також сусідні свічки (через `.shift()`).

    Бібліотека `pandas_ta` використовується для обрахунку популярних свічкових патернів
    (наприклад, engulfing, doji, hammer, star).

    Args:
        df (pd.DataFrame): OHLCV-датафрейм з обов’язковими колонками:
            ['open', 'high', 'low', 'close', 'volume']

    Returns:
        pd.DataFrame: Новий датафрейм з доданими колонками патернів. Назви колонок
        відповідають назвам патернів у snake_case (наприклад, 'bullish_engulfing',
        'high_wave', 'pin_bar', 'inside_bar', 'doji_star', 'rel_candle_size').

    Raises:
        ValueError: Якщо вхідний DataFrame не містить обов’язкових колонок
        або має некоректний формат.

    Патерни, які підтримуються:
        - Односвічкові: long_body, doji, hammer, inverted_hammer, spinning_top, marubozu
        - Двосвічкові: bullish_engulfing, bearish_engulfing, harami, piercing
        - Трисвічкові: morning_star, evening_star, three_white_soldiers, three_black_crows
        - Кастомні: pin_bar, high_wave, firefly, inside_bar, outside_bar, strong_trend_candle
        - Показники: rel_candle_size, shadow_ratio, candle_body_strength
        - Потенційні формації: ascending_triangle (експериментально)

    Примітки:
        - Більшість патернів потребують кількох попередніх свічок, тому будуть NaN
          на перших рядках.
        - Всі патерни конвертовані у числові значення (0/1), щоб забезпечити
          сумісність з ML-пайплайнами.
        - Метод зберігає count доданих ознак та може вести лог подій.

    Приклади:
        df_with_patterns = pattern_generator.add_candle_pattern_features(df_ohlcv)
    """
    ...
    def create_custom_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        """
    Створює набір спеціалізованих індикаторів, які відображають структуру ринку криптовалют.

    Метод аналізує надані OHLCV-дані та розраховує низку індикаторів, що враховують
    поведінку ціни, об'єму, трендів, фракталів, рівнів Фібоначчі, а також властиві для
    крипторинку особливості, такі як накопичення/розподіл, об'ємні аномалії та фазові цикли.

    Підтримується розрахунок індикаторів у трьох категоріях:
        - 'basic': індикатори на основі лише ціни (`close`)
        - 'volume': індикатори, що вимагають ціну та об'єм (`close`, `volume`)
        - 'ohlcv': індикатори, що вимагають всі OHLCV дані (`open`, `high`, `low`, `close`, `volume`)

    Метод автоматично перевіряє наявність необхідних колонок та обробляє відсутність певних індикаторних груп.

    Args:
        data (pd.DataFrame): Вхідний DataFrame з щонайменше колонками OHLCV:
            - 'open', 'high', 'low', 'close', 'volume'

    Returns:
        pd.DataFrame: Новий DataFrame з доданими колонками-індикаторами.
    """
