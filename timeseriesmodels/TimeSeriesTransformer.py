...
class TimeSeriesTransformer:
    ...
    def transform_data(self, data: pd.Series, method: str = 'log') -> Union[pd.Series, Tuple[pd.Series, float]]:
        """
            Застосовує трансформацію до часових рядів для стабілізації дисперсії та нормалізації розподілу.

            Метод підтримує кілька типів трансформацій, включаючи логарифмічну, квадратного кореня, Box-Cox, Yeo-Johnson,
            диференціювання та залишення даних без змін. Також враховується обробка негативних та нульових значень, що
            важливо для коректного застосування математичних перетворень.

            Підтримувані методи трансформації:
                - 'none' — без трансформації;
                - 'log' — логарифмічна (зсув для уникнення нуля/від'ємних значень);
                - 'sqrt' — квадратний корінь (зсув для уникнення від'ємних значень);
                - 'boxcox' — трансформація Бокса-Кокса (потрібні лише додатні значення);
                - 'yeo-johnson' — трансформація Йео-Джонсона (дозволяє від'ємні значення);
                - 'diff' — диференціювання ряду.

            У випадку трансформацій, які потребують інверсії, параметри (offset, lambda, transformer) зберігаються в self.transformations.

            Args:
                data (pd.Series or pd.DataFrame): Вхідний часовий ряд. Якщо DataFrame, буде взята перша колонка.
                method (str): Метод трансформації. Один із ['none', 'log', 'sqrt', 'boxcox', 'yeo-johnson', 'diff'].

            Returns:
                Union[pd.Series, Tuple[pd.Series, float]]: Трансформований ряд.
                    - Для 'boxcox' і 'yeo-johnson' повертається кортеж (ряд, параметр λ).
                    - Для інших — лише Series.

            Raises:
                ValueError: Якщо вказано невідомий метод трансформації.

            Логування:
                - Попередження про наявність NaN, нульових або від’ємних значень.
                - Інформація про зсув або параметри трансформації.
                - Помилки у випадку невдалої трансформації (з fallback до логарифмічної).
            """
