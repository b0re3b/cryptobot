...
class TimeSeriesAnalyzer:
    ...
    def detect_seasonality(self, data: pd.Series) -> Dict:
        """
    Виявляє сезонність у часовому ряді за допомогою автокореляційного аналізу, декомпозиції та спектрального аналізу.

    Метод виконує комплексну перевірку часових рядів на наявність повторюваних циклів (сезонності), що особливо важливо
    для фінансових та економічних даних. Застосовуються такі техніки:
      - ACF/PACF для визначення автокореляцій,
      - Декомпозиція часових рядів (Seasonal Decompose),
      - Спектральний аналіз через періодограму.

    Підсумком є список потенційних сезонних періодів з оцінкою їхньої достовірності та джерелами підтвердження.

    Рекомендовані вхідні колонки:
        - `original_close` — для необроблених цін;
        - `close_log` — для логарифмованих значень (рекомендовано для фінансових рядів).

    Args:
        data (pd.Series): Часовий ряд у вигляді pandas.Series з числовими значеннями та індексом часу.

    Returns:
        Dict: Словник з результатами аналізу сезонності, що включає:
            - 'status' (str): Статус виконання ('success' або 'error').
            - 'message' (str): Повідомлення про результат або помилку.
            - 'has_seasonality' (bool): Ознака наявності сезонності.
            - 'primary_period' (int, optional): Основний виявлений сезонний період.
            - 'seasonal_periods' (List[Dict]): Список виявлених періодів з полями:
                - 'period' (int): Значення періоду.
                - 'confidence' (float): Оцінка достовірності (0.0–1.0).
                - 'sources' (List[str]): Джерела підтвердження (наприклад, 'acf', 'spectral', 'decomposition').
            - 'details' (Dict): Деталізована інформація про окремі етапи аналізу (ACF, декомпозиція, спектральний аналіз).

    Raises:
        None — всі винятки обробляються всередині методу, логуються та повертаються у вигляді статусу 'error'.
    """
