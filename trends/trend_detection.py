...
class TrendDetection:
    ...
    def detect_trend_reversal(self, data: pd.DataFrame, recent_periods: int = 100) -> List[Dict]:
        """
          Виявляє потенційні розвороти тренду на основі технічних індикаторів та свічкових патернів.

          Метод аналізує останні `recent_periods` рядків історичних цінових даних, щоб виявити можливі сигнали розвороту тренду.
          Застосовуються ковзні середні (SMA), RSI, MACD, смуги Боллінджера, а також свічкові патерни (поглинання, молот).
          Повертає список словників, які описують кожен виявлений сигнал розвороту (тип, дата, підтвердження тощо).

          Parameters
          ----------
          data : pd.DataFrame
              OHLCV-таблиця історичних цінових даних. Має містити щонайменше колонки:
              'open', 'high', 'low', 'close'.
          recent_periods : int, optional
              Кількість останніх періодів для аналізу. За замовчуванням 100.

          Returns
          -------
          List[Dict]
              Список словників із інформацією про потенційні сигнали розвороту тренду.
              Кожен словник може містити такі ключі, як:
                  - 'date': дата сигналу
                  - 'type': тип патерну або індикатору ('bullish_engulfing', 'bearish_engulfing', 'hammer', тощо)
                  - 'confirmation': чи підтверджений сигнал іншими індикаторами (True/False)
                  - 'trend_context': поточний стан тренду за SMA (наприклад, 'uptrend' або 'downtrend')
          """
    ...
    def detect_chart_patterns(self, data: pd.DataFrame, recent_periods: int = 200) -> List[Dict]:
        """
    Виявляє графічні цінові патерни (Double Top, Double Bottom, трикутники) на основі останніх даних.

    Метод аналізує історичні дані OHLC, шукає ключові точки розвороту (swing points),
    та ідентифікує класичні графічні фігури технічного аналізу:
    - Подвійне дно (Double Bottom)
    - Подвійна вершина (Double Top)
    - Висхідний трикутник (Ascending Triangle)
    - Низхідний трикутник (Descending Triangle)

    Для визначення патернів використовуються локальні мінімуми/максимуми, схожість рівнів цін, і зміни між ними.

    Parameters
    ----------
    data : pd.DataFrame
        OHLC-дані з колонками 'high', 'low', 'close'. Індекс або стовпець 'date' повинен бути datetime-типу,
        або індексом, який можна конвертувати в дату.
    recent_periods : int, optional
        Кількість останніх періодів, які використовуються для пошуку патернів (default=200).

    Returns
    -------
    List[Dict]
        Список словників, кожен з яких описує знайдений графічний патерн.
        Формат словника може включати:
            - 'type': тип патерну ('double_bottom', 'double_top', 'ascending_triangle', 'descending_triangle')
            - 'start_date': дата початку формації
            - 'end_date': дата завершення формації
            - Додаткові параметри залежно від типу, наприклад:
                - 'bottom1', 'bottom2', 'middle_peak' для Double Bottom
                - 'top1', 'top2', 'middle_trough' для Double Top
                - 'resistance_level' або 'support_level' для трикутників
            - 'strength': оцінка сили патерну (від 0 до 1)
            - 'recent': логічне значення, чи знаходиться патерн в останніх даних
    ...
    def detect_divergence(self, price_data: pd.DataFrame, indicator_data: pd.DataFrame) -> List[Dict]:
        """
            Виявляє бичачі та ведмежі дивергенції між ціною та технічним індикатором.

            Цей метод ідентифікує як регулярні (негативні/позитивні), так і приховані дивергенції,
            порівнюючи локальні екстремуми (максимуми та мінімуми) в цінових даних та даних індикатора.
            Дивергенції можуть сигналізувати про можливі розвороти або продовження тренду.

            Параметри
            ---------
            price_data : pd.DataFrame
                DataFrame із ціновими даними, який має містити принаймні колонку 'close'.
                Індекс повинен бути у форматі datetime і відповідати індексу `indicator_data`.

            indicator_data : pd.DataFrame
                DataFrame з даними технічного індикатора.
                Для порівняння буде використано першу колонку. Індекс має співпадати з `price_data`.

            Повертає
            --------
            List[Dict]
                Список словників, кожен з яких описує виявлену дивергенцію.
                Кожен словник містить такі ключі:
                - 'type': str, тип дивергенції ('regular_bearish', 'regular_bullish', 'hidden_bullish', 'hidden_bearish')
                - 'price_date1': datetime, дата першого цінового екстремуму
                - 'price_date2': datetime, дата другого цінового екстремуму
                - 'price_value1': float, значення ціни на першому екстремумі
                - 'price_value2': float, значення ціни на другому екстремумі
                - 'indicator_value1': float, значення індикатора на першому екстремумі
                - 'indicator_value2': float, значення індикатора на другому екстремумі
                - 'description': str, опис типу дивергенції

            Винятки
            -------
            ValueError
                У разі відсутності колонки 'close' у price_data або якщо indicator_data порожній.
            """
